        ...
        <div className='flex flex-col gap-[14px]'>
          {
            ...
            .map(book => {
              return <div key={book._id} className='w-full bg-[#0f172acc] rounded-[14px] flex gap-[14px]'>
                <img src={`${book.book.bookImage}`} alt="book image" style={{borderRadius: '14px 0 0 14px'}} className='h-[250px] w-[180px]'/>
                <div className='w-full flex flex-col gap-[3px]' style={{marginRight: '12px', marginBottom: '8px'}}>
                  ...
                  <p><span className='text-[12px] font-bold'>Due Date:</span> <span className={`text-[12px] font-bold ${new Date(book.dueDate).getTime() < new Date().getTime()? 'text-red-400':'text-green-400'}`}>{new Date(book.dueDate).toLocaleString()}</span></p>
